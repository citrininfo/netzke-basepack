{
  ...
  initComponent: function(){
    ...
    if(this.rememberSelection && this.refreshSelection){
      ...
      this.getView().on('beforerefresh', this.refreshSelection, this);
    }
  ...
  buildStore: function() {
    var store = Ext.create('Ext.data.Store', Ext.apply({
      ...
      buffered: this.enableInfiniteScroll,
      leadingBufferZone: this.leadingBufferZone || 200,
      autoLoad: !this.loadInlineData
    }, this.dataStore));

    delete this.dataStore;

    if(this.enableInfiniteScroll == true){
      store.getNewRecords= function() {
        return [];
      };
      store.data.findBy= function(fn, scope, start) {
        var me = this,
        result = -1;
        if(start == undefined)
          start = 0;
        ...
        if (index >= start && fn.call(scope, rec)) {
          result = rec;
          return false;
          }
        });
      ...
      store.getUpdatedRecords= function() {
        result = [];
        var start = 0;
        while((rec = this.data.findBy(function(r){return Object.getOwnPropertyNames(r.modified).length > 0}, this, start)) !== -1){
          result.push(rec);
          //console.dir(start);
          if(start > 0)
            break;
          var start = rec.index+2;
        }
        return result;
  ...
  */
  normalizeAssociationRenderer: function(c) {
    ...
    c.renderer = function(value, a, r, ri, ci, store, view){
      var column = this.query('gridcolumn:not([hidden])')[ci],
